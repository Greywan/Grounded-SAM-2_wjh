...
def main(video_dir, output_dir, step, logger):
    ...
    # sam_mask_path = os.path.join(output_dir, 'mask_sam2')
    # CommonUtils.creat_dirs(sam_mask_path)
    ...
    inference_state = video_predictor.init_state(img_path=img_paths[0], video_path=video_dir, offload_video_to_cpu=True, async_loading_frames=True)
     # the step to sample frames for Grounding DINO predictor
    ...
    # prompt grounding dino to get the box coordinates on specific frame
        ...
        processing_order, clear_non_cond_mem, batch_size = video_predictor.propagate_in_video_prepare_wjh(inference_state, img_paths, max_frame_num_to_track=step, start_frame_idx=start_frame_idx)
        
        for out_frame_idx in tqdm(processing_order, desc="propagate in video"):
            img_path = img_paths[out_frame_idx]
            output = video_predictor.propagate_in_video_wjh(inference_state, 
...
if __name__ == '__main__':
    ...
    output_dir = "./outputs/byd/bydlight_step1_the30_iou03_videomask"
