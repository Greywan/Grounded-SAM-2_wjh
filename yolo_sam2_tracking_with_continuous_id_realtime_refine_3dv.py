...
from mask_propocess_for3dv import MaskProcessor
def main(video_dir, output_dir, step, logger):
    ...
    init_frame = cv2.imread(img_paths[0])
    init_frame = cv2.cvtColor(init_frame, cv2.COLOR_BGR2RGB)
    init_frame = cv2.resize(np.array(init_frame), (1024, 576))
    inference_state = maskProcessor.init_sam2_video_state(img=init_frame, 
    ...
    for frame_idx in tqdm(range(len(frame_names))):
        ...
        if frame_idx != 0:
            output = sam2_video_predictor.propagate_in_video_wjh(inference_state, 
                                                                frame_idx, colorImage, clear_non_cond_mem,batch_size)
        ...
        yolo_outputs = yolo_preprocess(colorImage[:, :, ::-1], input_points,input_labels,
        ...
        yolo_filtermasks_draw = draw_sam2_masksboxes(colorImage[:, :, ::-1], yolo_masks, input_boxes,font_scale=0.4)
        ...
        for object_id, object_info in mask_dict.labels.items():
            frame_idx, out_obj_ids, out_mask_logits = sam2_video_predictor.add_new_mask_prompt(
                    inference_state,
                    start_frame_idx,
                    object_id,
                    object_info.mask,
        ...
        (processing_order, clear_non_cond_mem, 
         batch_size) = sam2_video_predictor.propagate_in_video_prepare_wjh(inference_state, 
                                                                            max_frame_num_to_track=step, start_frame_idx=start_frame_idx)
...
if __name__ == '__main__':
    ...
    output_dir = "./outputs/byd/bydlight_filtermask_1219forjob" # ori name bydlight_0925_filtermask
