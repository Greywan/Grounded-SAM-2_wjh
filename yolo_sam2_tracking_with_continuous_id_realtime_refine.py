...
from utils.filter import filter_overlapping_boxes,filter_overlapping_masks
import json
import copy
import pdb
import time
...
def main(video_dir, output_dir, step, logger):
    ...
    mask_yolo_single_dir = os.path.join(output_dir, "mask_yolo_single")
    CommonUtils.creat_dirs(mask_yolo_single_dir)

    mask_result_single_dir = os.path.join(output_dir, "mask_result_single")
    CommonUtils.creat_dirs(mask_result_single_dir)

    mask_sam2_single_dir = os.path.join(output_dir, "mask_sam2_single")
    CommonUtils.creat_dirs(mask_sam2_single_dir)

    mask_sam2_single_data_dir = os.path.join(output_dir, "mask_sam2_single_data")
    CommonUtils.creat_dirs(mask_sam2_single_data_dir)
    ...
    # prompt grounding dino to get the box coordinates on specific frame
        ...
        input_boxes = results_filteroutput["boxes"]
        yolo_masks = results_filteroutput["masks"]
        # process the detection results
        OBJECTS = results_filteroutput["labels"]

        # filtered_boxes_indices = filter_overlapping_masks(input_boxes, containment_threshold=0.78)
        # filtered_boxes_indices = np.arange(len(results_filteroutput["labels"]))
        # input_boxes = input_boxes[filtered_boxes_indices]
        # OBJECTS = OBJECTS[filtered_boxes_indices]
        # yolo_masks_single = results_filteroutput["masks_single"][filtered_boxes_indices]
        # for i, yolo_mask_single in enumerate(yolo_masks_single):
        #     yolo_mask_single_draw = np.zeros((576, 1024, 3), dtype=np.uint8)
        #     obj_ids = np.unique(yolo_mask_single)
        #     indices = np.nonzero(obj_ids)
        #     obj_id = int(obj_ids[indices[0]])
        ...
        yolo_filtermasks_single_path = os.path.join(yolo_filtermask_path, f"{frame_idx}_{image_base_name}" + '.jpg')
        yolo_filtermasks_draw = draw_masksandboxes(colorImage[:, :, ::-1], yolo_masks, input_boxes,font_scale=0.4)
        cv2.imwrite(yolo_filtermasks_single_path, yolo_filtermasks_draw)
        
        
        timer.start('sam2_img_predict_all')
        # prompt SAM 2 image predictor to get the mask for the object
        masks, scores, logits = maskProcessor.sam2_img_predict(colorImage, input_boxes)
        filtered_indices = filter_overlapping_masks(masks)
        masks = masks[filtered_indices]
        input_boxes = input_boxes[filtered_indices]
        OBJECTS = OBJECTS[filtered_indices]

        timer.end('sam2_img_predict_all')
        # for idx, mask in enumerate(masks): # 有问题
        #     mask_vis = np.zeros((576, 1024), dtype=np.uint8)
        #     mask_vis[mask == 1] = 255
        #     obj_id = filtered_boxes_indices[idx]
        #     mask_vis_save_path = os.path.join(mask_sam2_single_dir, f"{frame_idx:04d}_{image_base_name}_{obj_id}" + '.jpg')
        #     cv2.imwrite(mask_vis_save_path, mask_vis.astype(np.uint8))
        # mask_sam2_np_path = os.path.join(mask_sam2_single_data_dir, f"{frame_idx:04d}_{image_base_name}" + '.npy')
        # np.save(mask_sam2_np_path, masks)
        ...
        for frame_idx, frame_masks_info in video_segments.items():
            ...
            for obj_id, obj_info in mask.items():
                ...
                # mask_draw = np.zeros((frame_masks_info.mask_height, frame_masks_info.mask_width, 3), dtype=np.uint8)
                # mask_where = mask_where.cpu().numpy()
                # mask_where_draw = np.zeros((frame_masks_info.mask_height, frame_masks_info.mask_width), dtype=np.uint8)
                # mask_where_draw[mask_where] = obj_id
                # mask_draw = draw_mask_specifycolor(mask_draw, mask_where_draw, font_scale=0.6)
                # image_base_name = frame_names[frame_idx].split(".")[0]
                # mask_single_path = os.path.join(mask_result_single_dir, f"{frame_idx:04d}_{image_base_name}_{obj_id}"+ ".jpg")
            ...
            img_draw = draw_masks_specifycolor(img_draw, mask_img,font_scale=0.4)
...
if __name__ == '__main__':
    ...
    video_dir = "data/byd/20240903_out"
    output_dir = "./outputs/byd/bydlight_0924_filtermask"
